...
ServerImpl::ServerImpl( 
    ...
    int max_num_threads,
    long timeout 
): Server(ps, pl), _max_num_threads(max_num_threads), _cur_num_threads(0), _timeout(timeout) {}
...
void ServerImpl::Join() {
    ...
    std::lock_guard<std::mutex> guard_sockets(_sockets_mu);
    for (auto client_socket: _client_sockets) {
        shutdown(client_socket, SHUT_RD);
    }

    std::unique_lock<std::mutex> lk(_stop_mu);

    _cv.wait(lk, [this]{ return _cur_num_threads == 0; });
...
void ServerImpl::OnRun() {
    ...
    while (running.load()) {
        ...
        {
            if (_cur_num_threads < _max_num_threads && running.load()) {
                ++_cur_num_threads;
                std::thread worker(&ServerImpl::process_connection, this, client_socket);
                worker.detach();
            } else {
                close(client_socket);
            }
...
void ServerImpl::process_connection(int client_socket) noexcept {
    std::unique_ptr<Execute::Command> command_to_execute;
    Protocol::Parser parser;
    std::size_t arg_remains;
    std::string argument_for_command;
    ...
    std::lock_guard<std::mutex> guard_stop(_stop_mu);
    --_cur_num_threads;
    if (_cur_num_threads == 0) {
        _cv.notify_one();
    }
}
